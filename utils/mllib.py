...
class UsedCarPriceModel():
    ...
    def __init__(self, model_path: str='./data/model.zlib', target_scaler_path: str='./data/target_scaler.zlib') -> None:
    ...
    def fit(self, 
            ...
            target_scaler_path: str='./data/target_scaler.zlib') -> None:
        """Retrain the model. 
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y_scaled, test_size=test_size, random_state=random_state, shuffle=suffle)
        ...
        model_score = {f"{metric} (training)": self.score(y_train, pred_train, squared=squared),
    ...
    def predict(self, cars: list[CarFeatures]) -> dict:
        ...
        return {'Prediction': prediction,
                'Unscaled prediction': prediction_unscaled
        }


    @classmethod
    def save(self, model_path: str='./data/model.zlib', target_scaler_path: str='./data/target_scaler.zlib') -> None:
        """Persist the model."""
    ...
    @staticmethod
    def transform(data: pd.DataFrame, current_year: int=2021) -> tuple(pd.DataFrame, np.ndarray):
        """Clean & Preprocess the database.
        See Used_Car_Price_Prediction.ipynb"""
        # Correct manufacter value
        data['manufacter'].replace({'audi': 'Audi', 'bmw': 'BMW', 'ford': 'Ford', 'hyundi': 'Hyundai', 'merc': 'Mercedes', 
        ...
        logging.debug({"Features shape": X.shape,
